...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        # if temp is lower than setpoint, pulse to fade in and out 
        # else if above light is on 
        # return to stop 
    ...
    def setupSerialOutput(self):
        current_state_str = self.current_state.id
        current_temper_f = floor(self.getFahrenheit())
        output = f"{current_state_str},{current_temper_f},{self.setPoint}\n"
        return output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%m/%d %H:%M:%S").Ijust(16)
    
            ## Setup Display Line 2
            if(altCounter < 6):
                current_temper_f = floor(self.getFahrenheit())
                lcd_line_2 = f"Current temp is: {current_temper_f}" 

    
                altCounter = altCounter + 1
            else:
                current_state_str = self.current_state.id
                lcd_line_2 = f"{current_state_str} Set:{self.setPoint}F"
